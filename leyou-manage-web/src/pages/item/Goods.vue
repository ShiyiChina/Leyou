    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="editsale(props.item)">下架</v-btn>
          <v-btn icon v-else @click="editsale(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        ...
        .then(() => {
          ...
          this.$message.confirm("商品信息不完整，无法查询");

          })
      },
      deleteGoods(goods){
        this.$message.confirm("确定要删除此商品吗？")
          .then(() =>{
            this.$http.delete("item/goods/delete/"+goods.id)
              .then(() =>{
      ...
      editsale(goods){
        if (goods.saleable == true) {
          this.$message.confirm("确定下架此商品？")
            .then(() =>{
              this.$http.put("item/goods/saleable",{
                id: goods.id,
                saleable: goods.saleable
              }).then(() =>{
